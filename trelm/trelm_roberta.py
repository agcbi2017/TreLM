...
from transformers.modeling_roberta import RobertaPreTrainedModel, RobertaLMHead, RobertaLayer, RobertaPooler, RobertaClassificationHead, create_position_ids_from_input_ids
...
class TrelmRobertaEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.tlayer = RobertaLayer(config) #
...
class TrelmRobertaEmbeddings(nn.Module):
...
class TrelmRobertaModel(RobertaPreTrainedModel):

    config_class = TrelmRobertaConfig
    base_model_prefix = "trelm_roberta"
    ...
    def __init__(self, config):
        ...
        self.embeddings = TrelmRobertaEmbeddings(config)
        self.encoder = TrelmRobertaEncoder(config)
    ...
    def forward(
        ...
        embedding_output = self.embeddings(
            input_ids=input_ids, lang_ids=lang_ids, position_ids=position_ids, token_type_ids=token_type_ids, inputs_embeds=inputs_embeds
        )

        #--Trelm--
        tlayer_position_ids = self.embeddings.create_position_ids_from_inputs_embeds(embedding_output)
        tlayer_position_embs = self.embeddings.position_embeddings(tlayer_position_ids)
        #--Trelm--

        encoder_outputs = self.encoder(
            embedding_output,
            tlayer_lang_embs=tlayer_lang_embs, # B x T x D
            tlayer_pos_embs=tlayer_position_embs, # B x T x D
...
class TrelmRobertaForMaskedLM(RobertaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `TrelmRobertaForMaskedLM` make sure `config.is_decoder=False` for "
...
class TrelmRobertaForSequenceClassification(RobertaPreTrainedModel):
...
@dataclass
class TrelmRobertaForPreTrainingOutput(ModelOutput):
...
class TrelmRobertaForPreTraining(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs
        ...
        return TrelmRobertaForPreTrainingOutput(
            loss=total_loss,
