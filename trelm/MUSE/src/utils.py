...
def read_txt_embeddings(params, source, full_vocab):
    ...
    logger.info("Loaded %i pre-trained word embeddings from txt." % len(vectors))
...
def load_pth_embeddings(params, source, full_vocab):
    ...
    assert embeddings.size() == (len(dico), params.emb_dim)
    logger.info("Loaded %i pre-trained word embeddings from pth." % len(dico))
...
def export_embeddings(src_emb, tgt_emb, params):
    ...
    if params.export == "pth":
        ...
        logger.info('Writing source embeddings to %s ...' % src_path)
        words_list = [params.src_dico[i] for i in range(len(params.src_dico))]
        torch.save({'dico': words_list, 'vectors': src_emb}, src_path) #params.src_dico
        logger.info('Writing target embeddings to %s ...' % tgt_path)
        words_list = [params.tgt_dico[i] for i in range(len(params.tgt_dico))]
        torch.save({'dico': words_list, 'vectors': tgt_emb}, tgt_path) # params.tgt_dico
