...
def build_model(params, with_dis):
    ...
    """
    # source embeddings
    src_dico, _src_emb = load_embeddings(params, source=True, full_vocab=True)
    ...
    # target embeddings
    if params.tgt_lang:
        tgt_dico, _tgt_emb = load_embeddings(params, source=False, full_vocab=True)
        params.tgt_dico = tgt_dico
        tgt_emb = nn.Embedding(len(tgt_dico), params.emb_dim, sparse=True)
        tgt_emb.weight.data.copy_(_tgt_emb)
        # fix the target embedding
        if params.fixed_tgt_emb:
