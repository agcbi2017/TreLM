...
if __name__ == '__main__':
    ...
    trelm_roberta_model = TrelmRobertaForMaskedLM.from_pretrained(data_args.init_model_path)
    trelm_roberta_tokenizer = BertTokenizerFast.from_pretrained(data_args.init_model_path)

    if not data_args.fix_ahalf:
        # fix the below half self-attention parameters
        for layer_idx in range(trelm_roberta_model.trelm_roberta.encoder.tlayer_position):
            for param in trelm_roberta_model.trelm_roberta.encoder.layer[layer_idx].parameters():
                param.requires_grad = False
    else:
        # fix the above half self-attention parameters
        for layer_idx in range(trelm_roberta_model.trelm_roberta.encoder.tlayer_position, len(trelm_roberta_model.trelm_roberta.encoder.layer)):
            for param in trelm_roberta_model.trelm_roberta.encoder.layer[layer_idx].parameters():
                param.requires_grad = False

    logger.info(trelm_roberta_model)

    logger.info('Evaluating trelm-roberta for refernece ...')
    pretrain_and_evaluate(training_args, data_args, trelm_roberta_model, trelm_roberta_tokenizer, eval_only=True, model_path=None)

    logger.info(f'Pretraining trelm-roberta ... ')
    pretrain_and_evaluate(training_args, data_args, trelm_roberta_model, trelm_roberta_tokenizer, eval_only=False, model_path=training_args.output_dir)

    model_path = training_args.output_dir
    
    logger.info(f'Saving model to {model_path}')
    trelm_roberta_model.save_pretrained(model_path)
    trelm_roberta_tokenizer.save_pretrained(model_path)